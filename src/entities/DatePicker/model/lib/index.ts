...
export const getPreviousMonthDays = (year: number, month: number) => {
  ...
  const prevMonthCellsAmount: number = getDayOfTheWeek(currentMonthFirstDay)
  ...
  for (let i: number = prevMonthCellsAmount - 1; i >= 0; i--) {
    dateCells.push({
      year: cellYear,
      month: cellMonth,
...
// export function inLastDay(cell: DateCellItem, todayDate: Date) {
//
//   const yesterdayDate: Date = new Date(todayDate)
//   yesterdayDate.setDate(todayDate.getDate() - 1)
//
//   if (isToday(cell, todayDate)) return false
//
//   return yesterdayDate > new Date(cell.year, cell.month, cell.date)
// }
